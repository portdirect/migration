...
generate_service_cirt () {
 ...
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa certprofile-show ampqUserCirt" | (
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa certprofile-show --out /tmp/service-cirt.cfg caIPAserviceCert"
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "sed -i \"/^desc=/c\\desc=This certificate profile is for authenticating AMPQ service users\" /tmp/service-cirt.cfg"
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "sed -i \"/^name=/c\\name=AMPQ Authenticated Client Certificate\" /tmp/service-cirt.cfg"
 ...
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "sed -i \"/^policyset.serverCertSet.7.default.params.exKeyUsageOIDs=/c\\policyset.serverCertSet.7.default.params.exKeyUsageOIDs=1.3.6.1.5.5.7.3.2\" /tmp/service-cirt.cfg"

 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "sed -i \"/^profileId=/d\" /tmp/service-cirt.cfg"
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa certprofile-import ampqUserCirt --file /tmp/service-cirt.cfg --desc \"AMPQ User Certificates\" --store TRUE"
)




docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa group-show ampq_users" || docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa group-add ampq_users"
docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa caacl-show ampq_user_acl" || docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa caacl-add ampq_user_acl"
docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa caacl-add-user ampq_user_acl --group ampq_users" || true
docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa caacl-add-profile ampq_user_acl --certprofile ampqUserCirt" || true
 ...
 if [ "${IPA_SVC_IP}" != "${OPENSTACK_KUBE_SVC_IP}" ]; then
   docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-del $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --del-all"
