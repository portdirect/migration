#!/bin/bash
set -e
OPENSTACK_COMPONENT="IPA-KEYTAB_MANAGER"
...
################################################################################
OPENSTACK_KUBE_SVC_NAME="password"
OPENSTACK_KUBE_SVC_NAMESPACE="os-accounts"
OPENSTACK_KUBE_SVC_IP="$(dig +short ${OPENSTACK_KUBE_SVC_NAME}.gantry.svc.${OS_DOMAIN})"
echo $OPENSTACK_KUBE_SVC_IP
...
retreive_service_cirt () {
  ...
  if docker exec ${FREEIPA_CONTAINER_NAME} ls ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d).keytab ; then
     docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "base64 --wrap=0 ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d).keytab" > ${KUBE_SVC_KEY_LOC}
  else
      echo "Command failed"
      return 1
  fi
...
generate_service_cirt () {
  ...
  docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa-getkeytab -s \$(hostname -s).$(hostname -d) -p host/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d) -k ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d).keytab"
  docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa-getkeytab -s \$(hostname -s).$(hostname -d) -p HTTP/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d) -k ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d).keytab"
...
update_service_ip () {
  ...
  ################################################################################
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "echo \"${IPA_USER_ADMIN_PASSWORD}\" | kinit $IPA_USER_ADMIN_USER"
 ( docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-show $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --raw" | grep "arecord" | awk '{printf $NF}' | head -1 ) || \
    docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-add $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --a-rec=${OPENSTACK_KUBE_SVC_IP}"
 IPA_SVC_IP=$(docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-show $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --raw" | grep "arecord" | awk '{printf $NF}' | head -1)
 if [ "${IPA_SVC_IP}" != "${OPENSTACK_KUBE_SVC_IP}" ]; then
   docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-del $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --del-all" || true
   docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-add $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --a-rec=${OPENSTACK_KUBE_SVC_IP}"
 fi
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "kdestroy"
}
...
generate_kube_secret_def () {
  ...
  ################################################################################
cat > ${SVC_AUTH_ROOT_LOCAL_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}-keytab-secret.yaml <<EOF
apiVersion: v1
kind: Secret
metadata:
  name: ${OPENSTACK_KUBE_SVC_NAME}-keytab-secret
  namespace: ${OPENSTACK_KUBE_SVC_NAMESPACE}
type: Opaque
data:
  host: $( echo "${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d)" | base64 --wrap=0 )
  keytab: $( cat ${KUBE_SVC_KEY_LOC} | base64 --wrap=0 )
EOF
}
...
create_kube_service_cirt_and_secret () {
  ...
  ################################################################################
  ( retreive_service_cirt && update_service_ip ) || generate_service_cirt
  generate_kube_secret_def
  kubectl_helper create -f ${SVC_AUTH_ROOT_LOCAL_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}-keytab-secret.yaml
  rm -rf ${SVC_AUTH_ROOT_LOCAL_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}-keytab-secret.yaml
  kubectl_helper describe secret/${OPENSTACK_KUBE_SVC_NAME}-keytab-secret --namespace=${OPENSTACK_KUBE_SVC_NAMESPACE}
}
...
delete_kube_secret () {
  ...
  get_service_cirt
  generate_kube_secret_def
  kubectl_helper delete -f ${SVC_AUTH_ROOT_LOCAL_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}-keytab-secret.yaml
}
