...
crudini --set $cfg bay k8s_atomic_template_path "/opt/magnum-templates/kubernetes/kubecluster.yaml"

# Location of template to build a k8s cluster on CoreOS. (string value)
crudini --set $cfg bay k8s_coreos_template_path = "/opt/magnum-templates/kubernetes/kubecluster-coreos.yaml"
...
crudini --set $cfg bay swarm_atomic_template_path "/opt/magnum-templates/swarm/swarmcluster.yaml"

# Location of template to build a Mesos cluster on Ubuntu. (string value)
crudini --set $cfg bay mesos_ubuntu_template_path "/opt/magnum-templates/mesos/mesoscluster.yaml"
...
################################################################################
find /opt/magnum-templates -type f -exec bash -c 'sed -i "s,{{IPA_CA_CRT}},$(cat /etc/pki/tls/certs/ca-bundle.crt | base64 --wrap 0)", "$0"' {} \;
chown -R magnum /opt/magnum-templates
