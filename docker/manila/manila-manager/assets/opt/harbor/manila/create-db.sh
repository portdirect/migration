...
################################################################################
check_required_vars MANILA_DB_PASSWORD MANILA_DB_NAME MANILA_DB_USER \
...
mysql -h ${MARIADB_SERVICE_HOST} \
      ...
      mysql <<EOF
CREATE DATABASE IF NOT EXISTS ${MANILA_DB_NAME} DEFAULT CHARACTER SET utf8 ;
GRANT ALL PRIVILEGES ON ${MANILA_DB_NAME}.* TO '${MANILA_DB_USER}'@'%' IDENTIFIED BY '${MANILA_DB_PASSWORD}' REQUIRE X509 ;
EOF
...
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Checking Service Dependencies"
################################################################################
fail_unless_db ${MANILA_DB_NAME}


################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Initializing the DB"
################################################################################
su -s /bin/sh -c "manila-manage --debug db sync" manila


################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Patching DB for Keystone V3"
################################################################################
mysql -h ${MARIADB_SERVICE_HOST} \
      -u root \
      -p"${DB_ROOT_PASSWORD}" \
      ...
      mysql <<EOF
use ${MANILA_DB_NAME} ;
ALTER TABLE share_networks MODIFY user_id VARCHAR(64);
EOF
...
################################################################################
su -s /bin/sh -c "manila-manage --debug db version" manila
