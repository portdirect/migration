...
// Returns the original service name for, allowing more cutomisationof the haproxy config
func getServiceNameForLBRulePassthrough(s *api.Service) string {
		return s.Name
}
// Returns the original service name for, allowing more cutomisationof the haproxy config
func getServiceNameForLBRulePublic(s *api.Service) string {
		r, _ := regexp.Compile("^os-")
		return fmt.Sprintf("%v", r.ReplaceAllString(s.Name, ""))
}
	...
	for _, s := range services.Items {
		...
		for _, servicePort := range s.Spec.Ports {
			...
			if servicePort.Protocol == api.ProtocolUDP {
				glog.Infof("Ignoring UDP %v: %+v", sName, servicePort)
				continue
			}

			if port, ok := lbc.targetService[sName]; ok && port == servicePort.Port {
				glog.Infof("Using for HTTP %v: %+v", sName, servicePort)
			} else {
				if port, ok := lbc.tcpServices[sName]; ok && port == servicePort.Port {
					glog.Infof("Using for TCP %v: %+v", sName, servicePort)
				} else {
			...
			newSvc := service{
				...
				ServiceName: getServiceNameForLBRulePassthrough(&s),
				PublicName: getServiceNameForLBRulePublic(&s),
				Ep:          ep,
...
func newLoadBalancerController(cfg *loadBalancerConfig, kubeClient *unversioned.Client, namespace string, targetService map[string]int, tcpServices map[string]int) *loadBalancerController {
...
func parseHTTPServices(targetService string) map[string]int {
	httpSvcs := make(map[string]int)
	for _, service := range strings.Split(targetService, ",") {
		portSplit := strings.Split(service, ":")
		if len(portSplit) != 2 {
			glog.Errorf("Ignoring misconfigured HTTP service %v", service)
			continue
		}
		if port, err := strconv.Atoi(portSplit[1]); err != nil {
			glog.Errorf("Ignoring misconfigured HTTP service %v: %v", service, err)
			continue
		} else {
			glog.Infof("Adding HTTP service %v", service)
			httpSvcs[portSplit[0]] = port
		}
	}
...
func main() {
	...
	var httpSvcs map[string]int
	if *targetService != "" {
		httpSvcs = parseHTTPServices(*targetService)
	} else {
		glog.Infof("No http services specified")
	}
	...
	lbc := newLoadBalancerController(cfg, kubeClient, namespace, tcpSvcs, httpSvcs)
