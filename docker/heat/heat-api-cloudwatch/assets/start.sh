...
crudini --set $cfg heat_api_cloudwatch cert_file "/etc/pki/tls/certs/ca.crt"
crudini --set $cfg heat_api_cloudwatch key_file "/etc/pki/tls/private/ca.key"
crudini --set $cfg heat_api_cloudwatch max_header_line "32768"

################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Launching"
################################################################################
exec /usr/bin/heat-api-cloudwatch --debug
