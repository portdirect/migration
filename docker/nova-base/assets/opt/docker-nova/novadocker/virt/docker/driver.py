...
from novadocker.virt.docker import utils as docker_driver_utils
...
class DockerDriver(driver.ComputeDriver):
    ...
    def _start_container(self, container_id, instance, network_info=None, devices=None):
        ...
        self.docker.start(container_id, binds=binds, dns=dns, cap_add=['SYS_ADMIN'],
                          privileged=CONF.docker.privileged, devices=devices)
    ...
    def get_volume_connector(self, instance):
        if self._initiator is None:
            self._initiator = docker_driver_utils.get_iscsi_initiator()
            if not self._initiator:
                LOG.debug('Could not determine iscsi initiator name',
                          instance=instance)

        if self._fc_wwnns is None:
            self._fc_wwnns = docker_driver_utils.get_fc_wwnns()
        ...
        if self._fc_wwpns is None:
            self._fc_wwpns = docker_driver_utils.get_fc_wwpns()
