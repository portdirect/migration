...
CONF = cfg.CONF
CONF.register_opts(volume_opts, 'docker')
...
class DockerISCSIVolumeDriver(DockerBaseVolumeDriver):
    ...
    def __init__(self, connection):
        super(DockerISCSIVolumeDriver, self).__init__(connection,
                                                      is_block_dev=True)
        self.num_scan_tries = CONF.docker.num_iscsi_scan_tries
        self.use_multipath = CONF.docker.iscsi_use_multipath
        if CONF.docker.iscsi_iface:
            self.transport = CONF.docker.iscsi_iface
        else:
