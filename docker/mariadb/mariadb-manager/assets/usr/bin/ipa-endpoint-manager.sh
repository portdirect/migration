...
################################################################################
OPENSTACK_KUBE_SVC_NAME="database"
...
generate_service_cirt () {
  ...
  ################################################################################
 docker -H unix:///run/docker.sock exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "echo \"${IPA_USER_ADMIN_PASSWORD}\" | kinit $IPA_USER_ADMIN_USER"
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-show $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --raw" || \
 ...
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "echo \"${IPA_USER_ADMIN_PASSWORD}\" | kinit $IPA_USER_ADMIN_USER; ipa host-show ${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d)" || \
  ...
  docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa host-add-managedby ${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d) --hosts=\$(hostname --fqdn)" || true

 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa service-add mysql/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d)"
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa service-add-host mysql/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d) --hosts=\$(hostname --fqdn)"
 ...
 -K mysql/${OPENSTACK_KUBE_SVC_NAME}.$(hostname -d)"
