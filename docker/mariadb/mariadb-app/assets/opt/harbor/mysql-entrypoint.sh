...
################################################################################
echo "${OS_DISTRO}: Configure MySQL settings"
################################################################################
. /opt/harbor/config-mysql.sh

if [ -z "$(ls /var/lib/mysql)" -a "${1%_safe}" = 'mysqld' ]; then
  ################################################################################
...
EOSQL
        ...
        if [ "$MARIADB_USER" -a "$MARIADB_PASSWORD" ]; then
                ...
                if [ "$MARIADB_DATABASE" ]; then
                        echo "GRANT ALL ON $MARIADB_DATABASE.* TO '$MARIADB_USER'@'%' REQUIRE X509 ;" >> "$TEMP_FILE"
                fi
        fi
