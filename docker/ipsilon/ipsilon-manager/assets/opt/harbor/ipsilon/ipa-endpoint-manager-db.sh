...
################################################################################
OPENSTACK_KUBE_SVC_NAME="ipsilon-db"
OPENSTACK_KUBE_SVC_NAMESPACE="os-ipsilon"
OPENSTACK_KUBE_SVC_IP="$(dig +short ${OPENSTACK_KUBE_SVC_NAME}.${OPENSTACK_KUBE_SVC_NAMESPACE}.svc.${OS_DOMAIN})"
...
generate_kube_secret_def () {
  ...
  ################################################################################
cat > ${SVC_AUTH_ROOT_LOCAL_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}-psql-ssl-secret.yaml <<EOF
apiVersion: v1
kind: Secret
metadata:
  name: ${OPENSTACK_KUBE_SVC_NAME}-psql-ssl-secret
...
create_kube_service_cirt_and_secret () {
  ...
  generate_kube_secret_def
  kubectl_helper create -f ${SVC_AUTH_ROOT_LOCAL_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}-psql-ssl-secret.yaml
  rm -rf ${SVC_AUTH_ROOT_LOCAL_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}-psql-ssl-secret.yaml
  kubectl_helper describe secret/${OPENSTACK_KUBE_SVC_NAME}-psql-ssl-secret --namespace=${OPENSTACK_KUBE_SVC_NAMESPACE}
}
...
delete_kube_secret () {
  ...
  get_service_cirt
  generate_kube_secret_def
  kubectl_helper delete -f ${SVC_AUTH_ROOT_LOCAL_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}-psql-ssl-secret.yaml
}
