...
################################################################################
check_required_vars IPSILON_TRANS_DB_PASSWORD IPSILON_TRANS_DB_NAME IPSILON_TRANS_DB_USER \
...
psql -h ${PSQL_SERVICE_HOST} \
      --user root \
      --no-password <<EOF
CREATE USER ${IPSILON_TRANS_DB_USER} WITH PASSWORD '${IPSILON_TRANS_DB_PASSWORD}';
EOF
psql -h ${PSQL_SERVICE_HOST} \
      --user root \
      --no-password <<EOF
CREATE DATABASE ${IPSILON_TRANS_DB_NAME};
EOF
psql -h ${PSQL_SERVICE_HOST} \
      --user root \
      --no-password <<EOF
GRANT ALL PRIVILEGES ON DATABASE ${IPSILON_TRANS_DB_NAME} TO ${IPSILON_TRANS_DB_USER} ;
EOF
