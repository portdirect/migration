...
################################################################################
check_required_vars IPSILON_ADMIN_DB_PASSWORD IPSILON_ADMIN_DB_NAME IPSILON_ADMIN_DB_USER \
...
psql -h ${PSQL_SERVICE_HOST} \
      --user root \
      --no-password <<EOF
CREATE USER ${IPSILON_ADMIN_DB_USER} WITH PASSWORD '${IPSILON_ADMIN_DB_PASSWORD}';
EOF
psql -h ${PSQL_SERVICE_HOST} \
      --user root \
      --no-password <<EOF
CREATE DATABASE ${IPSILON_ADMIN_DB_NAME};
EOF
psql -h ${PSQL_SERVICE_HOST} \
      --user root \
      --no-password <<EOF
GRANT ALL PRIVILEGES ON DATABASE ${IPSILON_ADMIN_DB_NAME} TO ${IPSILON_ADMIN_DB_USER} ;
EOF
