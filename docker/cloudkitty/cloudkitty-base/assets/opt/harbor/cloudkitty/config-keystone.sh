...
check_required_vars cfg KEYSTONE_ADMIN_SERVICE_HOST KEYSTONE_API_VERSION \
                    SERVICE_TENANT_NAME CLOUDKITTY_KEYSTONE_USER \
                    CLOUDKITTY_KEYSTONE_PASSWORD KEYSTONE_API_VERSION
...
crudini --set $cfg keystone_authtoken auth_url "${KEYSTONE_AUTH_PROTOCOL}://${KEYSTONE_ADMIN_SERVICE_HOST}:35357/v3"
...
crudini --set $cfg keystone_authtoken username "${CLOUDKITTY_KEYSTONE_USER}"
crudini --set $cfg keystone_authtoken password "${CLOUDKITTY_KEYSTONE_PASSWORD}"
crudini --set $cfg keystone_authtoken auth_version "v${KEYSTONE_API_VERSION}"
crudini --set $cfg keystone_authtoken region "${DEFAULT_REGION}"
...
export OS_USERNAME="${CLOUDKITTY_KEYSTONE_USER}"
export OS_PASSWORD="${CLOUDKITTY_KEYSTONE_PASSWORD}"
