...
################################################################################
check_required_vars CLOUDKITTY_DB_PASSWORD CLOUDKITTY_DB_NAME CLOUDKITTY_DB_USER \
...
mysql -h ${MARIADB_SERVICE_HOST} \
      ...
      mysql <<EOF
CREATE DATABASE IF NOT EXISTS ${CLOUDKITTY_DB_NAME} DEFAULT CHARACTER SET utf8 ;
GRANT ALL PRIVILEGES ON ${CLOUDKITTY_DB_NAME}.* TO '${CLOUDKITTY_DB_USER}'@'%' IDENTIFIED BY '${CLOUDKITTY_DB_PASSWORD}' REQUIRE X509 ;
EOF
...
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Checking Service Dependencies"
################################################################################
fail_unless_db ${CLOUDKITTY_DB_NAME}


################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Initializing the DB"
################################################################################
su -s /bin/sh -c "cloudkitty-dbsync --debug upgrade" ${CLOUDKITTY_USER}
#su -s /bin/sh -c "cloudkitty-storage-init --debug" ${CLOUDKITTY_USER}
