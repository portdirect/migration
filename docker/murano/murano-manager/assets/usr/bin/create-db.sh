...
################################################################################
check_required_vars MURANO_DB_PASSWORD MURANO_DB_NAME MURANO_DB_USER \
...
mysql -h ${MARIADB_SERVICE_HOST} \
      ...
      mysql <<EOF
CREATE DATABASE IF NOT EXISTS ${MURANO_DB_NAME} DEFAULT CHARACTER SET utf8 ;
GRANT ALL PRIVILEGES ON ${MURANO_DB_NAME}.* TO '${MURANO_DB_USER}'@'%' IDENTIFIED BY '${MURANO_DB_PASSWORD}' REQUIRE X509 ;
EOF
