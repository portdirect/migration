...
# ################################################################################
# echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Creating $IPA_REALM admin project"
# ################################################################################
# IPA_REALM_DESCRIPTION="${IPA_REALM} admin project"
# openstack project create --domain ${IPA_REALM} --description "${IPA_REALM_DESCRIPTION}" --enable ${IPA_REALM} \
#     || openstack --os-identity-api-version 3 \
#           project show --domain ${IPA_REALM} ${IPA_REALM}
...
################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Making sure that the $IPA_USER_ADMIN_USER@$IPA_REALM is admin of the ${IPA_REALM} domain"
################################################################################
openstack user show ${IPA_ADMIN_USER_ID}
openstack role add --domain ${IPA_REALM}  --user ${IPA_ADMIN_USER_ID} admin
#openstack role add --project-domain ${IPA_REALM} --project ${IPA_REALM} --user-domain ${IPA_REALM} --user ${IPA_USER_ADMIN_USER} admin
