...
retreive_service_cirt () {
  ...
  if docker exec ${FREEIPA_CONTAINER_NAME} ls ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.ssl.key ; then
     docker exec ${FREEIPA_CONTAINER_NAME} cat ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.ssl.key > ${KUBE_SVC_KEY_LOC}
  else
      echo "Command failed"
      return 1
  fi
  if docker exec ${FREEIPA_CONTAINER_NAME} ls ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.ssl.crt ; then
     docker exec ${FREEIPA_CONTAINER_NAME} cat ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.ssl.crt> ${KUBE_SVC_CRT_LOC}
  else
      echo "Command failed"
      return 1
  fi
...
generate_service_cirt () {
 ...
 docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-show $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --raw" || docker exec ${FREEIPA_CONTAINER_NAME} /bin/bash -c "ipa dnsrecord-add $(hostname -d) ${OPENSTACK_KUBE_SVC_NAME} --a-rec=${OPENSTACK_KUBE_SVC_IP}"
 ...
 -f ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.ssl.crt \
 -k ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.ssl.key \
...
get_service_cirt () {
  ( retreive_service_cirt && update_service_ip ) || ( generate_service_cirt &&  retreive_service_cirt )
}
...
#delete_kube_secret
(kubectl_helper describe secret/${OPENSTACK_KUBE_SVC_NAME}-ssl-secret --namespace=${OPENSTACK_KUBE_SVC_NAMESPACE} && retreive_service_cirt ) || \
  create_kube_service_cirt_and_secret
