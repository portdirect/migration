...
GRANT ALL PRIVILEGES ON ${KEYSTONE_DB_NAME}.* TO '${KEYSTONE_DB_USER}'@'%' IDENTIFIED BY '${KEYSTONE_DB_PASSWORD}' REQUIRE X509 ;
EOF
...
################################################################################
su -s /bin/bash -c "keystone-manage --debug --verbose db_sync" keystone

################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Getting the DB version"
################################################################################
su -s /bin/bash -c "keystone-manage --verbose db_version" keystone
