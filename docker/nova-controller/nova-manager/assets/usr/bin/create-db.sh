...
################################################################################
check_required_vars NOVA_DB_PASSWORD NOVA_DB_NAME NOVA_API_DB_NAME NOVA_DB_USER \
...
GRANT ALL PRIVILEGES ON ${NOVA_DB_NAME}.* TO '${NOVA_DB_USER}'@'%' IDENTIFIED BY '${NOVA_DB_PASSWORD}' REQUIRE X509 ;
EOF
...
GRANT ALL PRIVILEGES ON ${NOVA_API_DB_NAME}.* TO '${NOVA_DB_USER}'@'%' IDENTIFIED BY '${NOVA_DB_PASSWORD}' REQUIRE X509 ;
EOF
...
fail_unless_db ${NOVA_API_DB_NAME}


################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Initializing the DB"
################################################################################
su -s /bin/bash -c "nova-manage --debug db sync" nova
su -s /bin/bash -c "nova-manage --debug api_db sync" nova


################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Getting the DB version"
################################################################################
su -s /bin/bash -c "nova-manage db version" nova
