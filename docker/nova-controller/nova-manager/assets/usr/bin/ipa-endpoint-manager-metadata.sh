...
# Nova metadata Settings
: ${NOVA_METADATA_SERVICE_HOSTNAME:="metadata.os-nova.svc"}
...
################################################################################
OPENSTACK_KUBE_SVC_NAME="metadata"
OPENSTACK_KUBE_SVC_NAMESPACE="os-nova"
...
kind: Secret
metadata:
  name: nova-${OPENSTACK_KUBE_SVC_NAME}-ssl-secret
...
create_kube_service_cirt_and_secret () {
  ...
  kubectl_helper describe secret/nova-${OPENSTACK_KUBE_SVC_NAME}-ssl-secret --namespace=${OPENSTACK_KUBE_SVC_NAMESPACE}
}
