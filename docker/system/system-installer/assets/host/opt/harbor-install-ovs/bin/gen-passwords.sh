...
rm -f /etc/harbor/${OS_DOMAIN}.pem
(
cat /etc/harbor/${OS_DOMAIN}.crt /etc/harbor/${OS_DOMAIN}.key > /etc/harbor/${OS_DOMAIN}.pem && \
openssl rsa -in /etc/harbor/${OS_DOMAIN}.key -check -noout && \
openssl x509 -in /etc/harbor/${OS_DOMAIN}.crt -text -noout
) || (
COCKPIT_ID=$(docker ps | grep docker.io/port/system-cockpit:latest | awk '{ print $1 }') && \
docker exec ${COCKPIT_ID} cat /etc/cockpit/ws-certs.d/0-self-signed.cert > /etc/harbor/${OS_DOMAIN}.pem
) || touch /etc/harbor/${OS_DOMAIN}.pem
...
mkdir -p /etc/harbor
cat > /etc/harbor/auth.env.tmp << EOF
...
EOF
ls -lah /etc/harbor/auth.env || cp /etc/harbor/auth.env.tmp /etc/harbor/auth.env
