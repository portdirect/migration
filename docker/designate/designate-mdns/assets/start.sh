...
ip -o addr | awk '!/^[0-9]*: ?lo|link\/ether/ {gsub("/", " "); print $2" "$4}' | grep ${DESIGNATE_MDNS_SERVICE_SVC_IP} || ( echo "This node has not got an ip address that kubernetes is aware of" && exit 1 )
...
################################################################################
check_required_vars cfg DESIGNATE_MDNS_SERVICE_SVC_IP


################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Setting IP to bind to"
################################################################################
crudini --set $cfg service:mdns host "${DESIGNATE_MDNS_SERVICE_SVC_IP}"
...
################################################################################
exec su -s /bin/sh -c "exec /usr/bin/designate-mdns --debug" designate
