...
mysql -h ${MARIADB_SERVICE_HOST} \
      ...
      mysql <<EOF
CREATE DATABASE IF NOT EXISTS ${DESIGNATE_POOL_DB_NAME} DEFAULT CHARACTER SET utf8 ;
GRANT ALL PRIVILEGES ON ${DESIGNATE_POOL_DB_NAME}.* TO '${DESIGNATE_POOL_DB_USER}'@'%' IDENTIFIED BY '${DESIGNATE_POOL_DB_PASSWORD}' REQUIRE X509 ;
EOF
...
################################################################################
su -s /bin/sh -c "designate-manage --debug pool-manager-cache sync" designate


################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Getting DB Version"
################################################################################
su -s /bin/sh -c "designate-manage --debug pool-manager-cache version" designate
