...
curl --cacert /etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem \
    ...
    --fail \
    https://ipsilon.${OS_DOMAIN}/idp/admin/providers/saml2/admin/sp/grafana/delete || true

    cd /etc/httpd/mellon
    echo ${IPA_HOST_ADMIN_PASSWORD} | ipsilon-client-install --debug \
    --hostname grafana.${OS_DOMAIN} \
...
cat /etc/httpd/mellon/certificate.pem |docker exec -i ${FREEIPA_CONTAINER_NAME} /bin/bash -c "cat > ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.saml.crt"
cat /etc/httpd/mellon/certificate.key |docker exec -i ${FREEIPA_CONTAINER_NAME} /bin/bash -c "cat > ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.saml.key"
cat /etc/httpd/mellon/metadata.xml |docker exec -i ${FREEIPA_CONTAINER_NAME} /bin/bash -c "cat > ${SVC_AUTH_ROOT_IPA_CONTAINER}/${OPENSTACK_KUBE_SVC_NAME}/${OPENSTACK_KUBE_SVC_NAME}.saml.xml"
  ...
  # --saml-sp-description "primary auth for grafana" \
  # --saml-sp-visible \
  # --debug
  # )
}
