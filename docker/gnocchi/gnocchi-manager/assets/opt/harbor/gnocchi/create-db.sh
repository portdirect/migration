...
################################################################################
check_required_vars GNOCCHI_DB_PASSWORD GNOCCHI_DB_NAME GNOCCHI_DB_USER \
...
GRANT ALL PRIVILEGES ON ${GNOCCHI_DB_NAME}.* TO '${GNOCCHI_DB_USER}'@'%' IDENTIFIED BY '${GNOCCHI_DB_PASSWORD}' REQUIRE X509 ;
EOF
...
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Checking Service Dependencies"
################################################################################
fail_unless_db ${GNOCCHI_DB_NAME}


################################################################################
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Initializing the DB"
################################################################################
su -s /bin/sh -c "gnocchi-upgrade --debug --verbose" ${GNOCCHI_USER}
