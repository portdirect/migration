...
################################################################################
check_required_vars GRAFANA_DB_PASSWORD GRAFANA_DB_NAME GRAFANA_DB_USER \
...
mysql -h ${MARIADB_SERVICE_HOST} \
      ...
      --ssl-verify-server-cert \
      mysql <<EOF
CREATE DATABASE IF NOT EXISTS ${GRAFANA_DB_NAME} DEFAULT CHARACTER SET utf8 ;
GRANT ALL PRIVILEGES ON ${GRAFANA_DB_NAME}.* TO '${GRAFANA_DB_USER}'@'%' IDENTIFIED BY '${GRAFANA_DB_PASSWORD}' ;
EOF
mysql -h ${MARIADB_SERVICE_HOST} \
      -u root \
      -p"${DB_ROOT_PASSWORD}" \
...
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Checking Service Dependencies"
################################################################################
fail_unless_db ${GRAFANA_DB_NAME}
