...
################################################################################
check_required_vars FOREMAN_DB_PASSWORD FOREMAN_DB_NAME FOREMAN_DB_USER \
...
mysql -h ${MARIADB_SERVICE_HOST} \
      ...
      mysql <<EOF
CREATE DATABASE IF NOT EXISTS ${FOREMAN_DB_NAME} DEFAULT CHARACTER SET utf8 ;
GRANT ALL PRIVILEGES ON ${FOREMAN_DB_NAME}.* TO '${FOREMAN_DB_USER}'@'%' IDENTIFIED BY '${FOREMAN_DB_PASSWORD}' ;
EOF
...
echo "${OS_DISTRO}: ${OPENSTACK_COMPONENT}: ${OPENSTACK_SUBCOMPONENT}: Checking Service Dependencies"
################################################################################
fail_unless_db ${FOREMAN_DB_NAME}
