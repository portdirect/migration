...
################################################################################
check_required_vars HORIZON_DB_PASSWORD HORIZON_DB_NAME HORIZON_DB_USER \
...
mysql -h ${MARIADB_SERVICE_HOST} \
      ...
      mysql <<EOF
CREATE DATABASE IF NOT EXISTS ${HORIZON_DB_NAME} DEFAULT CHARACTER SET utf8 ;
GRANT ALL PRIVILEGES ON ${HORIZON_DB_NAME}.* TO '${HORIZON_DB_USER}'@'%' IDENTIFIED BY '${HORIZON_DB_PASSWORD}' REQUIRE X509 ;
EOF
