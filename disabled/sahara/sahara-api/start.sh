...
PUBLIC_ENDPOINT_URL="${KEYSTONE_AUTH_PROTOCOL}://${SAHARA_API_SERVICE_HOST}:8386/v1.1/%(tenant_id)s"
INTERNAL_ENDPOINT_URL="${KEYSTONE_AUTH_PROTOCOL}://${SAHARA_API_SERVICE_HOST}:8386/v1.1/%(tenant_id)s"
ADMIN_ENDPOINT_URL="${KEYSTONE_AUTH_PROTOCOL}://${SAHARA_API_SERVICE_HOST}:8386/v1.1/%(tenant_id)s"
(
  (
    (
...
exec /usr/bin/sahara-api --config-file /etc/sahara/sahara.conf
